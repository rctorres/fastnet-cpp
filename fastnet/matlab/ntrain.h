 ...
 */
  void saveTrainInfo(unsigned epoch, REAL trnError, REAL tstError)
  {
    TrainData trainData;
...
{
  MatEvents *inTrnData, *outTrnData, *inValData, *outValData;
  vector<MatEvents*> inTrnList, inValList;
  ...
  {  
    ...
    bool patRecNet;
    unsigned numPat;
    inTrnData = outTrnData = inValData = outValData = NULL;
    if ( (!mxIsEmpty(args[OUT_TRN_IDX])) && (!mxIsEmpty(args[OUT_TST_IDX])) )
    {
      ...
      inValData = new MatEvents (args[IN_TST_IDX]);
      outValData = new MatEvents (args[OUT_TST_IDX]);
    }
    else // It is a pattern recognition network.
    {
      ...
      for (unsigned i=0; i<numPat; i++)
      {
        ...
        inValList.push_back(new MatEvents (mxGetCell(args[IN_TST_IDX], i)));
  ...
  //Until we make Backpropagation training multithreaded, we canno use it.
    ...
    if (!patRecNet)
    {
      ...
      REPORT("Total number of testing events      : " << inValData->getNumEvents());
    }
    else
    {
      ...
      for (unsigned i=0; i<numPat; i++)
      {
        ...
        REPORT("Total number of testing events      : " << inValList[i]->getNumEvents());
      }
    }
    ...
    {
      minValError = static_cast<REAL>(5E20);
      tstText = ", mse (test) = ";
      comp = smallerThan;
    }
    ...
    for (unsigned epoch=0; epoch<nEpochs; epoch++)
    {
      if (!patRecNet)
      {
        ...
        tstError = testNetwork(net, inValData, outValData);
      }
      else
      {
        trnError = trainNetwork(net, inTrnList, outList, trnEpochList);
        tstError = testNetwork(net, inValList, outList, epochValOutputs);
        if (useSP) tstError = sp(outList, epochValOutputs);
      }

      // Saving the best weight result.
      if ((*comp)(tstError,minValError))
      {
        net->saveBestTrain();
        minValError = tstError;
  ...
  catch(bad_alloc xa)
  {
    ...
    if (inValData) delete inValData;
    if (outValData) delete outValData;
    for (unsigned i=0; i<inTrnList.size(); i++)  delete inTrnList[i];
    for (unsigned i=0; i<inValList.size(); i++)  delete inValList[i];
