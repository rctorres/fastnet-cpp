function r = relevance(net, data)
%function r = relevance(net, data)
%Perform relevance analysis. net can be iether a projection vector/matrix,
%or a neural network structure. data can be a matrix, in which case the
...
%
  ...
  if iscell(data),
    ...
    r = do_by_mse(net, data);
  end


function r = do_by_mse(net, data)
...
function r = do_by_sp(net, data)
  %Does relevance by SP.
  nROC = 200;
  nDim = size(data{1},1);
  r = zeros(1,nDim);
  sp_ref = max(genROC(get_output(net, data{1}), get_output(net, data{2}), nROC));
  mdata = mean(cell2mat(data), 2);
  
  for i=1:nDim,
     aux = data;
     aux{1}(i,:) = mdata(i);
     aux{2}(i,:) = mdata(i);
     sp = max(genROC(get_output(net, aux{1}), get_output(net, aux{2}), nROC));
     r(i) = sp_ref - sp;
  end
