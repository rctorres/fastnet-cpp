		...
		public:
			...
			*/
			void readWeights(REAL ***w, REAL **b) const
			{
				...
				{
					for (unsigned j=0; j<nNodes[0]; j++) w[0][i][j] = (REAL) iw(i,j);
					b[0][i] = (REAL) ib(i);
				}
			...
			*/
			void flushErrors(mxArray *&epoch, mxArray *&trnError, mxArray *&tstError) const
			{
				...
				epoch = mxCreateDoubleMatrix(1,size, mxREAL);
				trnError = mxCreateDoubleMatrix(1,size, mxREAL);
				tstError = mxCreateDoubleMatrix(1,size, mxREAL);

				double *ep = mxGetPr(epoch);
				double *trn = mxGetPr(trnError);
				double *tst = mxGetPr(tstError);
				
				while (itr != trnList.end())
				{
					aux = *itr++;
					ep[k] = static_cast<double>(aux.epoch);
					trn[k] = static_cast<double>(aux.trnError);
					tst[k] = static_cast<double>(aux.tstError);
					k++;
				}
			}
			...
			*/
			mxArray *flushOutputs() const
			{
				...
				const unsigned outSize = nNodes[nLayers-1];
				REAL *out;
				mxArray *ret = mxCreateDoubleMatrix(outSize, outList.size(), mxREAL);
				...
				{
					out = *itr++;
					for (unsigned i=0; i<outSize; i++) ptr(i,k) = static_cast<double>(out[i]);
					k++;
				}
